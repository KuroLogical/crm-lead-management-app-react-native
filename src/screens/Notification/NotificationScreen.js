...
<View style={NotificationStyles.container}>
  <Spacing space={20}/>
      <Text style={NotificationStyles.sectionHeader}>Today</Text>
      <FlatList
        data={notifications.slice(0, 3)}
        keyExtractor={(item) => item.id}
        renderItem={({ item }) => (
          <View style={NotificationStyles.notificationItem}>
            {/* <Ionicons name={item.icon} size={24} color="green" style={NotificationStyles.icon} /> */}
            <VectorIcon icon="Ionicons" color={Colors.theme_background} name={item.icon} size={24} />
      ...
      />
      <Text style={NotificationStyles.sectionHeader}>Yesterday</Text>
      <FlatList
