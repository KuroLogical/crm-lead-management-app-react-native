...
import { Colors, darkTheme, lightTheme, SF, SH } from "../../utils";
...
const LanguageScreen = (props) => {
  ...
  return (
    <ImageBackground source={images.LoginShap} style={LanguageStyles.Container}>
       <View style={LanguageStyles.TopView}>
          <Spacing space={60} />
          <Text style={LanguageStyles.Welcome_back_Text}>{t("Your")}</Text>
          <Text style={LanguageStyles.Password}>{t("Langauge")}</Text>
        </View>
      <Spacing space={SH(200)} />
      <FlatList 
        numColumns={2}
        style={LanguageStyles.MainPadding}
      ...
      <View style={LanguageStyles.MainPadding}>
        <Button title={Translation('Confirm')} onPress={() => {changeLanguage(selectedItems);navigation.navigate(RouteName.LOGINFIRSTSCREEN)}} />
      </View>
      <Spacing space={SH(50)} />
    </ImageBackground>
  );
